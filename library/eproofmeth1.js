...
function appendMethods_EProof__SID__ () {
  ...
  //#################################################################
	//# Nested: getTailHTML()
	//#################################################################
  this.getHeaderHTML = function(pJS,pCSS) {
    var vHeader = this.getValueDOM("TPLHTMLHEADER"+this.aQID);
  	var vJS = pJS || this.getValueDOM("TPLJSLIBS"+this.aQID);
  	var vCSS = pCSS || this.getValueDOM("TPLCSSFILES"+this.aQID);
  	vHeader = this.replaceString(vHeader,"___MATHJAX_PATH___",this.aMathJaxPath);
